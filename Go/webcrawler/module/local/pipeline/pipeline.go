...
func New(mid module.MID, itemProcessors []module.ProcessItem, scoreCalculator module.CalculateScore) (module.Pipeline, error) {
	...
	for i, pipelien := range itemProcessors {
		if pipelien == nil {
			err := genParameterError(fmt.Sprintf("nil item processoe[%d]", i))
			return nil, err
		}
		innerProcessors = append(innerProcessors, pipelien)
	}
	return &myPipeline{
...
func (pipeline *myPipeline) Send(item module.Item) []error {
	...
	for _, processor := range pipeline.itemProcessors {
		processorItem, err := processor(currentItem)
		if err != nil {
			errs = append(errs, err)
